...
public class Card extends Object{
    ...
    private Boolean isMultiple;//whether the answer is plain text or multiple choice
    ...
    public Card(Status status, int sessionsLeft, String question, String answer, String pictureName, Boolean isMultiple){
        setStatus(status);
    ...
    public Card(Status status,String question,String answer,String pictureName,Boolean isMultiple){
        this(status,status.getReps(),question,answer,pictureName,isMultiple);
    }
    
    public Card(String question,String answer,String pictureName,Boolean isMultiple){
        this(Status.DEFAULT_STATUS,question,answer,pictureName,isMultiple);
    }
    
    public Card(String question,String answer,Boolean isMultiple){
        this(question,answer,NO_PICTURE_STRING,isMultiple);
    }
    //contructors for radio type answer
    public Card(Status status, int sessionsLeft, String question, String wrong1, 
            String wrong2, String wrong3, String right, String pictureName, Boolean isMultiple){
        setStatus(status);
        this.sessionsLeft = sessionsLeft;
        
        this.questionText = question;
        this.wrongText1 = wrong1;
        this.wrongText2 = wrong2;
        this.wrongText3 = wrong3;
        this.rightText = right;
        this.pictureName = pictureName;
        this.isMultiple = isMultiple;
    }  
    
    public Card(Status status,String question,String wrong1, 
            String wrong2, String wrong3, String right,String pictureName,Boolean isMultiple){
        this(status,status.getReps(),question,wrong1,wrong2,wrong3,right,pictureName,isMultiple);
    }
    
    public Card(String question,String wrong1, 
            String wrong2, String wrong3, String right,String pictureName,Boolean isMultiple){
        this(Status.DEFAULT_STATUS,question,wrong1,wrong2,wrong3,right,pictureName,isMultiple);
    }
    
    public Card(String question,String wrong1, 
            String wrong2, String wrong3, String right, Boolean isMultiple){
        this(question,wrong1,wrong2,wrong3,right,NO_PICTURE_STRING,isMultiple);
    }
    ...
    public String getAnswer(){
      ...
      public String getRadioWrongAnswer(){
        return bringBackNewlines(wrongText1); //replace the newline replacer with the actual \n character       
        }
      
      public String getRadioWrongAnswer2(){
        return bringBackNewlines(wrongText2); //replace the newline replacer with the actual \n character       
        }
      
      public String getRadioWrongAnswer3(){
        return bringBackNewlines(wrongText3); //replace the newline replacer with the actual \n character       
        }
      
      public String getRadioRightAnswer(){
        return bringBackNewlines(rightText); //replace the newline replacer with the actual \n character       
        }
    ...
    public void setRadioWrongAnswer(String text){
        if(text != null && text.equals("")==false){
            this.wrongText1 = text; 
        }
    }

   public void setRadioWrongAnswer2(String text){
        if(text != null && text.equals("")==false){
            this.wrongText2 = text; 
        }
    }
   
   public void setRadioWrongAnswer3(String text){
        if(text != null && text.equals("")==false){
            this.wrongText3 = text; 
        }
    }
   
   public void setRadioRightAnswer(String text){
        if(text != null && text.equals("")==false){
            this.rightText = text; 
        }
    }
   ...
   public void setIsMultiple(Boolean isMultiple){
    ...
    public static Card createCardBasedOnText(String text){
        try{
            ...
            if(stuff[stuff.length - 1].equals("true")){
                Card card = new Card(
    ...
    public String toString(){
        ...
        String text = status.toString() + sessionsLeft + Card.DELIMITER
                    + questionText + Card.DELIMITER
                    + wrongText1 + Card.DELIMITER
                    + wrongText2 + Card.DELIMITER
                    + wrongText3 + Card.DELIMITER
                    + rightText + Card.DELIMITER
                    + pictureName + Card.DELIMITER
                    + isMultiple;
            //replace newlines with the newline replacer
            text = replaceNewlines(text);
            ...
            public boolean equals(Object aCard){
                ...
                if(aCard instanceof Card == false)return false; //if it's not a card, stop it
                    ...
                    if(card.answerText != null){
                        return card.answerText.equals(this.answerText) 
                                && card.questionText.equals(this.questionText)
                                && card.pictureName.equals(this.pictureName)
                                && card.status == this.status;
                        //everything must match
                    } else { 
                        if (card.wrongText2 == null && card.wrongText3 == null){
                            return card.wrongText1.equals(this.wrongText1) 
                                && card.rightText.equals(this.rightText)
                                && card.questionText.equals(this.questionText)
                                && card.pictureName.equals(this.pictureName)
                                && card.status == this.status
                                && card.isMultiple == this.isMultiple;
                        } else if (card.wrongText3 == null){
                            return card.wrongText1.equals(this.wrongText1) 
                                && card.wrongText2.equals(this.wrongText2) 
                                && card.rightText.equals(this.rightText)
                                && card.questionText.equals(this.questionText)
                                && card.pictureName.equals(this.pictureName)
                                && card.status == this.status
                                && card.isMultiple == this.isMultiple;
                        }else if(card.wrongText2 == null){
                            return card.wrongText1.equals(this.wrongText1) 
                                && card.wrongText3.equals(this.wrongText3)
                                && card.rightText.equals(this.rightText)
                                && card.questionText.equals(this.questionText)
                                && card.pictureName.equals(this.pictureName)
                                && card.status == this.status
    ...
    public int hashCode() {
        ...
        hash = 43 * hash + (this.wrongText1 != null ? this.wrongText1.hashCode() : 0);
        hash = 43 * hash + (this.wrongText2!= null ? this.wrongText2.hashCode() : 0);
        hash = 43 * hash + (this.wrongText3 != null ? this.wrongText3.hashCode() : 0);
        hash = 43 * hash + (this.rightText != null ? this.rightText.hashCode() : 0);
        hash = 43 * hash + (this.pictureName != null ? this.pictureName.hashCode() : 0);
        hash = 43 * hash + isMultiple.hashCode();
